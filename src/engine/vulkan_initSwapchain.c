    ...
    if(swapchainHasDepth){
        ...
        for(size_t i = 0; i < swapchainDepthImages.count; i++){
            vkCmdTransitionImage(
                tempCmd,
                swapchainDepthImages.items[i],
                VK_IMAGE_LAYOUT_UNDEFINED, 
                swapchainHasStencil ? VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL : VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL,
                swapchainHasStencil ? VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_DEPTH_BIT : VK_IMAGE_ASPECT_DEPTH_BIT
            );
        }
